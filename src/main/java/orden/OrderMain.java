...
public class OrderMain {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for (OrdenTipo tipo: OrdenTipo.values()) {
            ...
            File tipoArchivo = new File(tipo.name() + "-datos.txt");
            ...
            for (int i = 0; i < PRUEBA.length; ++i) {
                ...
                if (resultados[i] == null) {
                    ...
                    if (tipo == OrdenTipo.BITONIC) {
                        BitonicSort.probar(copia);
                    } else if (tipo == OrdenTipo.BURBUJA) {
                        Burbuja.probar(copia);
                    } else if (tipo == OrdenTipo.MERGE) {
                        MergeSort.probar(copia);
                    } else if (tipo == OrdenTipo.PIGEONHOLE) {
                        PigeonholeSort.probar(copia);
                    } else if (tipo == OrdenTipo.QUICK) {
                        Quicksort.probar(copia);
                    } else if (tipo == OrdenTipo.STOOGE) {
                        StoogeSort.probar(copia);
                    }
    ...
    private static void guardarResultamos() throws IOException {
        for (Map.Entry<OrdenTipo, Double[]> entrada: RESULTADOS.entrySet()) {
            ...
            File archivo = new File(tipo.name() + "-datos.txt");
